...
def dijkstra_search(start, goal, net):
    """
    Dijkstra's Search algorithm for finding the shortest path between two nodes in the SUMO network.
    """
    frontier = []
    ...
    while frontier:
        ...
        for edge in current_node.getOutgoing():
            ...
            if next_node.getID() not in cost_so_far or new_cost < cost_so_far[next_node.getID()]:
                ...
                heappush(frontier, (new_cost, next_node.getID()))
...
sumoCmd = ["sumo", "-c", "Sumo_Config/osm.sumocfg", "--routing-algorithm", "dijkstra", "--no-warnings", "true"]
traci.start(sumoCmd)
                ...
                if stops:
                    ...
                    if next_stop:
                        ...
                        if current_edge_id and goal_edge_id:
                            ...
                            came_from, cost_so_far = dijkstra_search(current_edge, goal_edge, net)
...
dataset.to_csv('SumoData/vehicle_data_dijkstra.csv', index=False)

person_dataset = pd.DataFrame(packPersonData, index=None, columns=person_columns)
person_dataset.to_csv('SumoData/person_data_dijkstra_.csv', index=False)