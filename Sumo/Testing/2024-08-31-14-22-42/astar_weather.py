    ...
    while frontier:
        ...
        for edge in current_node.getOutgoing():
            ...
            if next_node.getID() not in cost_so_far or new_cost < cost_so_far[next_node.getID()]:
                ...
                priority = new_cost + heuristic(next_node, weather_df)
...
dataset.to_csv('SumoData/vehicle_data_astar_weather.csv', index=False)

person_dataset = pd.DataFrame(packPersonData, index=None, columns=person_columns)
person_dataset.to_csv('SumoData/person_data_astar_weather.csv', index=False)